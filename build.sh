...
target=aarch64-linux-user,x86_64-linux-user
target=$target,aarch64-softmmu,x86_64-softmmu
...
if [ ! -f .configured ]; then
    ...
    opt-gcov)
        configure_flags=""
        export CFLAGS="-fno-omit-frame-pointer --coverage"
        export LDFLAGS="--coverage"
        ;;
    debug)
        configure_flags="--enable-debug --enable-asan --enable-ubsan"
        export CFLAGS="-fno-omit-frame-pointer"
        ;;
    tsan)
        if [ ! -d glib_tsan ]; then
    ...
    clang)
        ...
        ;;
    *)
        choices="debug, opt, opt-gcov, tsan, clang"
