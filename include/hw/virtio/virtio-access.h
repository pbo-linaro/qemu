...
{
#if HOST_BIG_ENDIAN
    return virtio_is_big_endian(vdev) ? s : bswap16(s);
#else
    return virtio_is_big_endian(vdev) ? bswap16(s) : s;
#endif
}
...
{
#if HOST_BIG_ENDIAN
    return virtio_is_big_endian(vdev) ? s : bswap32(s);
#else
    return virtio_is_big_endian(vdev) ? bswap32(s) : s;
#endif
}
...
{
#if HOST_BIG_ENDIAN
    return virtio_is_big_endian(vdev) ? s : bswap64(s);
#else
    return virtio_is_big_endian(vdev) ? bswap64(s) : s;
#endif
}
