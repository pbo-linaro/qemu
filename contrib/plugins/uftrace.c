...
static void uftrace_write_task(GArray *traces)
{
...
static void uftrace_write_info(GArray *traces)
{
     ...
     * uftrace file header: features      = 0x1263 (PLTHOOK | TASK_SESSION ...
     * uftrace file header: info          = 0x7bff (EXE_NAME | EXE_BUILD_ID ...
...
static callstack *callstack_clone(callstack *cs)
{
...
static void cpu_trace_last_sample(Cpu *cpu)
{
    if (!cpu->sample_cs) {
        return;
    }
    uint64_t elapsed = cpu->sample_insn_count;
    uint64_t last_sample_timestamp = cpu->insn_count - elapsed;
    uint64_t middle_timestamp = last_sample_timestamp + (elapsed / 2);
...
static void cpu_set_new_sample(Cpu *cpu)
{
...
static void track_privilege_change(unsigned int cpu_index, void *udata)
{
    ...
    uint64_t timestamp = cpu->insn_count;
    uint64_t fp = cpu->ops.get_frame_pointer(cpu);

    if (trace_sample) {
        cpu_trace_last_sample(cpu);
    }
                                           ...
                                           int argc, char **argv)
{
    ...
    if (!strcmp(info->target_name, "aarch64")) {
        ...
        fprintf(stderr, "this plugin supports only aarch64 target");
        exit(1);
    }
